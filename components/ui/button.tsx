...
export interface ButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'success' | 'ghost' | 'outline' | 'link';
  size?: 'sm' | 'md' | 'lg' | 'icon';
  isLoading?: boolean;
  loadingText?: string;
}
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'primary', size = 'md', isLoading, loadingText = 'Loading...', children, disabled, ...props }, ref) => {
    ...
    const variants = {
      ...
      secondary: 'border border-border bg-surface text-charcoal hover:bg-background hover:border-brand/50 focus:ring-brand/20',
      success: 'bg-success text-white hover:bg-success-dark focus:ring-success/20',
      ghost: 'text-charcoal hover:bg-muted hover:text-foreground',
